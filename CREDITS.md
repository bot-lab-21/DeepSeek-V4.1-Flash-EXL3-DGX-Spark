...
| **tonyd2wild** (DeepSeek-V4.1-Flash-vLLM-DGX-Spark, MIT) | 4× Spark recipe: 7 patches, worker-first boot, Engram-on-disk staging before forward, node-local Engram rows, bench protocol + prompt set v1, GPU slow-state probe (gpuflip), NCCL/RoCE env, image chain baseline | card, recipe doc, notes, gist | in use — our boot3 reproduces boot-10 |
