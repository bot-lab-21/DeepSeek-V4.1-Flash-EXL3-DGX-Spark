...
log "preflight OK (signature $sig)"; NP0=$(wc -l < $K/node_prep.log 2>/dev/null || echo 0); [ "${SKIP_PREP:-0}" = 1 ] || { PROFILE=$PROFILE IMAGE=$IMAGE FLUSH_GIB=${FLUSH_GIB:-40} bash $K/dsv41_node_prep_v2.sh $PREP_NODES 2>&1 | grep -E "NODE-PREP|FAIL|CUDA free|GPU state" | tee -a "$LOG"; tail -n +$((NP0+1)) $K/node_prep.log | grep -q "NODE-PREP-ALL-OK \[$PREP_NODES\]" || { log "ABORT: node prep failed"; exit 1; }; }; scp -q $K/dsv41_tp4_launch.sh ${N[0]}:~/ ; for r in 1 2 3; do scp -q $K/dsv41_tp4_launch.sh ${N[$r]}:~/; done
ENVS="LINE=$LINE MEM_MIN_GIB=${MEM_MIN_GIB:-0} EXTRA_ENV=\"${EXTRA_ENV:-}\" MAX_BATCHED=${MAX_BATCHED:-8192} SPEC_ADAPT=${SPEC_ADAPT:-false} ROCE_MAX=${ROCE_MAX:-2MB} MOE_BLOCK_M=${MOE_BLOCK_M:-} NCCL_ALGO=${NCCL_ALGO:-} NCCL_PROTO=${NCCL_PROTO:-} NCCL_NTHREADS=${NCCL_NTHREADS:-} RUST_FE=${RUST_FE:-0} ASYNC=${ASYNC:-0} EXP_NAME=$EXP_NAME IMAGE=$IMAGE KV_BYTES=${KV_BYTES:-} SERVED_ALIAS=${SERVED_ALIAS:-} HOT_DIR=${HOT_DIR:-} HOT_ROWS=${HOT_ROWS:-} ROCE=${ROCE:-} NCCL_CH=${NCCL_CH:-} ENGRAM_THREADS=${ENGRAM_THREADS:-32} GMU=$GMU MAXLEN=$MAXLEN SEQS=$SEQS EAGER=$EAGER SPEC=$SPEC SPEC_K=$SPEC_K ENGRAM_DISK=1 TEXT_ONLY=$TEXT_ONLY THINKING=$THINKING PARSERS=$PARSERS"
