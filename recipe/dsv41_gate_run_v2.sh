...
for row in $ROWS; do exp="${PROFILE}-${LABEL:-$row}${LINE:+-$LINE}"; env_kv=$(row_env $row)
  ...
  ( cd $K && env LINE=$LINE PROFILE=$PROFILE EXP_NAME=$exp $env_kv bash dsv41_boot_tp4_v2.sh ) 2>&1 | grep -E "DSV41-TP4|smoke:|warmup|toolcall|KV:|ABORT|NODE-PREP-(ALL|SOME)" | tee -a "$LOG"
