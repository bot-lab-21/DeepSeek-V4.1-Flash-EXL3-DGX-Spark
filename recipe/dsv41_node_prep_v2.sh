...
while true; do c=\$(awk "/^Cached:/{print int(\\\$2/1048576)}" /proc/meminfo); [ "\${c:-0}" -gt ${FLUSH_GIB:-40} ] && { sudo -n /usr/local/bin/glm-drop-caches >/dev/null 2>&1 || { sync; echo 3 | sudo -n tee /proc/sys/vm/drop_caches >/dev/null 2>&1; }; echo "$(date +%T) dropped at ${c}G"; }; sleep 5; done'
declare -A SIG; fail=0
for n in $NODES; do h=${N[$n]}; ok=1; log "== $n prep (profile $PROFILE)"
  FL=${FLUSHER//'${FLUSH_GIB:-40}'/${FLUSH_GIB:-40}}
...
chmod +x /tmp/cache_flusher.sh"
  ...
  busy=$(S $h 'docker ps --format "{{.Names}}" | grep -cE "^(vllm|dsv41cook|dsv41cap|dsv41fq)"'); if [ "${busy:-1}" = 0 ]; then
    ...
    bt=$(T=120 S $h "docker run --rm --gpus all --user \$(id -u):\$(id -g) -e HOME=/tmp -v /mnt/glm52:/mnt/glm52 --entrypoint python3 $IMAGE /mnt/glm52/dsv41kit/gputools/gpuburn.py 8 2>/dev/null | grep -oE '= [0-9.]+ TFLOPS' | grep -oE '[0-9.]+' | cut -d. -f1"); log "$n GPU burn: ${bt:-?} TFLOPS fp16"; [ "${bt:-0}" -ge ${BURN_MIN_TFLOPS:-50} ] 2>/dev/null || { log "$n FAIL GPU clock latch (${bt:-?} TFLOPS < ${BURN_MIN_TFLOPS:-50}; fix = AC unplug 30-60 s; idea: tonyd2wild gpu-clock-latch)"; ok=0; }
