...
class EmulationMxfp8LinearKernel(Mxfp8LinearKernel):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # --- woproj patch (DSV41_WOPROJ_B12X) -------------------------------
        # For BMM layers (DeepSeek-V4 ``wo_a``) keep the ORIGINAL MXFP8 payload
        # around before the dequant below replaces ``layer.weight`` with BF16.
        # ``flashinfer_sparse_woproj.py`` packs these into the native b12x
        # ``wo_projection`` MXFP8 GEMM. Plain tensors, no grad, no extra copy
        # (the fp8 storage is simply kept alive instead of being freed). Only
        # stash once (idempotent on weight reload / when a pre-hook already did).
        if (
            getattr(layer, "is_bmm", False)
            and weight.dtype == torch.float8_e4m3fn
            and weight_scale.dtype == MXFP8_SCALE_DTYPE
            and getattr(layer, "mxfp8_weight_fp8", None) is None
        ):
            layer.mxfp8_weight_fp8 = weight.detach().contiguous()
            layer.mxfp8_weight_scale_u8 = weight_scale.detach().contiguous()
        # --------------------------------------------------------------------
