...
_HOT_DIR = _kai_os.environ.get("DSV41_ENGRAM_HOT_DIR", "")          # bot-labs-21 P6: resident hot rows ("Pollard-guessed" from corpus hash frequencies)
_HOT_ROWS = int(_kai_os.environ.get("DSV41_ENGRAM_HOT_ROWS", "20000000"))
_HOT_LOG_EVERY = int(_kai_os.environ.get("DSV41_ENGRAM_HOT_LOG_EVERY", "2000"))
...
class DiskEngramTable:
    ...
    def load_hot(self, layer_id: int) -> None:
        """bot-labs-21 P6 (2026-09-10): make this rank's share of the corpus-frequency hot set resident. Reads
        DSV41_ENGRAM_HOT_DIR/engram_hot_L{layer}.safetensors (global row ids sorted by descending frequency), keeps the
        first DSV41_ENGRAM_HOT_ROWS that fall in this rank's [row_start, row_start+num_rows) range, and pulls their
        fp8 rows + ue8m0 scales into pinned host tensors with the same preadv path (one batch at init). Lookups then
        split unique ids into resident hits (torch gather) and disk misses (unchanged read path)."""
        self.hot_ids = None
        if not _HOT_DIR:
            return
        path = _kai_os.path.join(_HOT_DIR, f"engram_hot_L{layer_id:02d}.safetensors")
        if not _kai_os.path.exists(path):
            logger.warning("Engram hot set: %s missing; layer %d stays disk-only", path, layer_id)
            return
        from safetensors.torch import load_file
        ids = load_file(path)["hot_ids"][:_HOT_ROWS]
        local = ids[(ids >= self.row_start) & (ids < self.row_start + self.num_rows)] - self.row_start
        local = torch.unique(local)                                    # sorted, rank-local rel ids
        n = int(local.numel())
        if n == 0:
            return
        ...
        self.hot_stats = [0, 0, 0]                                     # lookups, hits, calls
        logger.info("Engram hot set: layer %d rank rows [%d, +%d): %d of the top %d resident (%.2f GiB pinned) in %.1fs",
                    layer_id, self.row_start, self.num_rows, n, _HOT_ROWS, n * (self.dim + self.sb) / 2**30, _kai_time.time() - t0)
...
def gather_dequant_many(requests: list) -> list[torch.Tensor]:
    ...
    for table, rel, owned in requests:
        ...
        hot = getattr(table, "hot_ids", None)
        if hot is not None and uniq.numel():
            # bot-labs-21 P6: resident hits by binary search, only the misses go to disk
            pos = torch.searchsorted(hot, uniq).clamp_(max=hot.numel() - 1)
            hit = hot[pos] == uniq
            if bool(hit.any()):
                w[hit] = table.hot_w[pos[hit]]
                s[hit] = table.hot_s[pos[hit]]
            miss = ~hit
            st = table.hot_stats
            st[0] += int(uniq.numel()); st[1] += int(hit.sum()); st[2] += 1
            if _HOT_LOG_EVERY and st[2] % _HOT_LOG_EVERY == 0:
                logger.info("Engram hot set: %.1f%% of %d unique row lookups resident over the last %d calls",
                            100.0 * st[1] / max(st[0], 1), st[0], st[2]); st[0] = st[1] = 0
            if bool(miss.any()):
                mi = miss.nonzero().squeeze(1)
                wm = torch.empty((mi.numel(), table.dim), dtype=torch.uint8)
                sm = torch.empty((mi.numel(), table.sb), dtype=torch.uint8)
                jobs += table.read_jobs(uniq[mi].tolist(), wm, sm)
                plans.append((table, w, s, inverse, owned, (mi, wm, sm)))
            else:
                plans.append((table, w, s, inverse, owned, None))
            continue
        jobs += table.read_jobs(uniq.tolist(), w, s)
        plans.append((table, w, s, inverse, owned, None))
    _kai_parallel_read(jobs)
    outs = []
    for table, w, s, inverse, owned, mi in plans:
        if mi is not None:                                             # scatter the disk misses next to the resident hits
            idx, wm, sm = mi
            w[idx] = wm; s[idx] = sm
