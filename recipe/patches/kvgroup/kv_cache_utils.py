...
def _get_packed_kv_cache_groups(
    ...
    # bot-lab-21 2026-09-11 (DSV41_KV_GROUPING=fine): the floor above makes the
    # widest mixed-page bucket define the pool block for *every* group. For
    # DeepSeek-V4.1 on SM12x (64-state pages: ratio-2 kv-source layers at
    # 128-token blocks, the ratio-1 layer at 64-token blocks) that is 3 x
    # (37440 + 8640) = 138240 B, while the ratio-1 group only fills 46080 B of
    # each block yet takes cdiv(L, 64) blocks per request (57% of a 1M request,
    # 2/3 of each block padding). Letting the repeat count go down to one
    # (compressed + indexer) pair per group makes the pool block 46080 B and
    # every group fill >= 81% of it: 3.41M -> 5.57M tokens at 1M / 12 GiB.
    # Default (env unset) keeps upstream behaviour byte-for-byte.
    _fine_grouping = os.environ.get("DSV41_KV_GROUPING", "") == "fine"
    if _fine_grouping and min_repeats_per_group:
        logger.info_once(
            "DSV41_KV_GROUPING=fine: packing KV cache groups at one pattern "
            "repeat per group (upstream floor was %d repeats).",
            min_repeats_per_group,
        )
    repeats_per_group = (
        _approximate_gcd(
            [
            ...
            lower_bound=1 if _fine_grouping else min_repeats_per_group,
        )
