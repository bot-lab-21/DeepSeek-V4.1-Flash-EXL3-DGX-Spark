...
lever_env(){ case $1 in G0) echo "";; RUST) echo "RUST_FE=1";; NCCLTREE) echo "NCCL_ALGO=Tree NCCL_PROTO=LL";; NTH256) echo "NCCL_NTHREADS=256";; NTH128) echo "NCCL_NTHREADS=128";; ROCE) echo "ROCE=1";; ADAPT) echo "SPEC_ADAPT=true";; MAXB16K) echo "MAX_BATCHED=16384";; ROCEMAX512K) echo "ROCE=1 ROCE_MAX=512KB";; ROCEMAX8M) echo "ROCE=1 ROCE_MAX=8MB";; BLOCKM16) echo "MOE_BLOCK_M=16";; BLOCKM32) echo "MOE_BLOCK_M=32";; FLUSH8) echo "FLUSH_GIB=8";; ET64) echo "ENGRAM_THREADS=64";; CH8) echo "NCCL_CH=8";; K7) echo "SPEC_K=7";; K4) echo "SPEC_K=4";; ASYNC) echo "ASYNC=1";; HOT) echo "HOT_DIR=/mnt/glm52/dsv41engram/hot90 HOT_ROWS=${HOT_ROWS:-20000000}";; SERVE1M) echo "MAXLEN=1000000 SEQS=8";; *) echo "$1";; esac; }
...
run_row(){ # $1 label, $2 env string → runs one gate row with the accepted env + this lever's env; returns 0 if bench done
  ...
  log "== rung $label: base [${acc:-none}] + lever [$envs]"; ROW_N0=$(wc -l < "$LOG")   # smoke lines for THIS row are the ones appended to $LOG from here on
  ( cd $K && env LINE=$LINE PROFILE=exl3 LABEL=$label QUALITY_ROWS="${QUALITY_ROWS:-G0}" $acc $envs bash dsv41_gate_run_v2.sh exl3 "$( [ -n "$envs" ] && echo "$envs" | tr ' ' ',' || echo G0 )" ) 2>&1 | grep -E "row |DSV41-TP4|smoke:|toolcall|KV:|ABORT|NODE-PREP-(ALL|SOME)|pass@1|ppl|needle" | cut -c1-180 | tee -a "$LOG"
...
for lv in $LEVERS; do e=$(lever_env $lv); label=$lv; ok=0
  ...
  if run_row $label "$e"; then read c1 c6 cp <<<"$(metrics exl3-$label-$LINE)"
    smoke=$(tail -n +$((ROW_N0+1)) "$LOG" | grep -cE "smoke: 1, 2, 3|toolcall: \['get_weather");   # was: grep -A3 on the shared boot log → always 0 → every rung rejected
